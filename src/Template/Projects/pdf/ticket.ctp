...
<div class="container">
    ...
    <h1 class="title is-mar-top-30">
        Ticket Création Projet - <?= $formatted_date ?>
    </h1>

    <h2 class="has-text-semibold is-mar-top-20 is-mar-bot-20 has-text-weight-semibold is-size-5"><strong>Créateur: <?= $user_account->user->user_fullname ?> - <?= $user_account->user->user_job ?></strong></h2>

    <h2 class="is-size-4 has-text-weight-semibold">Verdict</h2>
    <article style="border: 3px solid orange;" class="is-pad-full-10">
        <p>
            Après analyse il est retenu que le projet <?= $data['project_name'] ?> 
			 <?php if($project->project_criticity == 'critical'): ?>
			             <strong>sera audité par le pôle séccurity by Design</strong>  car il répond aux critères d'éligibilité édictés en la matière (se réferer plus bas pour en avoir connaissance, le cas échéant).
			 <?php else: ?>
   						 <strong>ne sera pas audité par le pôle séccurity by Design</strong>  car il ne répond pas aux critères d'éligibilité édictés en la matière (se réferer plus bas pour en avoir connaissance, le cas échéant).
			 <?php endif; ?>
    ...
    <form name="createProjectForm">
        ...
        <div class="field is-horizontal">
			...
			<?= $data['project_type_id']['project_type_denomination'] ?>
        </div>
        	...
        	<?php if(count($data['contributors'])>0): ?>
                            ...
                            <div class="field is-horizontal" id="actor_area">
                                ...
                                <div class="field-body">
                                    <div class="field" >
										<?php foreach($data['contributors'] as $key => $value): ?>
											<table class="table is-striped is-narrow is-hoverable is-fullwidth">
												<tbody>
													<tr class="actor_cadre" ng-repeat="u in users | filter:search_actors">
														<td>
																<img width="130px" src="img/assets/admins/photo/<?= $value['user_photo'] ?>">
														</td>
														<td>
															  <p>
														        <strong><?= $value['user_fullname'] ?></strong>
														        <br>
																<?= $value['user_email'] ?>
																<br>
																<?= $value['user_job'] ?> <br>
																Role: <?= $value['assigned_role']['role_denomination'] ?>
														      </p>
														</td>
													</tr>
												</tbody>
            ...
            <div class="field-body">
                ...
                <div class="field is-mar-bot-15">
                    <h3 class="subtitle has-text-weight-semibold">Connectivité Internet</h3>
                    	<?php if($data['indices']['project_is_internet_connected'] == 'true') :?>
                    		Oui
                    	<?php else: ?>
							Non
						<?php endif; ?>
                ...
                <div class="field is-mar-bot-5">
                    <h3 class="subtitle has-text-weight-semibold">Utilisation de données sensibles?</h3>
                    	<?php if($data['indices']['project_is_used_harmful_data'] == 'true') :?>
                    		Oui
                    	<?php else: ?>
							Non
                    	<?php endif; ?>
                ...
                <div class="field is-mar-bot-15">
                    <h3 class="subtitle has-text-weight-semibold">Connexions Tierces</h3>
                    	<?php if($data['indices']['project_is_used_third_connexions'] == 'true') :?>
                    		Oui
                    	<?php else: ?>
							Non
                    	<?php endif; ?>
            ...
            <div class="field-body">
                ...
                <div class="field is-mar-bot-15">
                    <h3 class="subtitle has-text-weight-semibold">Exposition Franchise</h3>
	                    	<?php if($data['indices']['project_is_franchise_exposed'] == 'true') :?>
	                    		Oui
	                    	<?php else: ?>
								Non
	                    	<?php endif; ?>
                ...
                <div class="field is-mar-bot-15">
                    <h3 class="subtitle has-text-weight-semibold">Service OCI et filiales?</h3>
	                    	<?php if($data['indices']['project_is_for_oci_and_subs'] == 'true') :?>
	                    		Oui
	                    	<?php else: ?>
								Non
	                    	<?php endif; ?>
                ...
                <div class="field is-mar-bot-15">
                    <h3 class="subtitle has-text-weight-semibold">Centralisation de données client?</h3>
	                    	<?php if($data['indices']['project_is_client_data_centralized'] == 'true') :?>
	                    		Oui
	                    	<?php else: ?>
								Non
	                    	<?php endif; ?>
